...
try {
	ASUser u = APPSENSOR.asUtilities().getCurrentUser();
	//org.owasp.appsensor.demoapp.AppSensorUser u = org.owasp.appsensor.demoapp.UserManager.getLoggedInUserObject(request);
	IntrusionRecord air = new AppSensorIntrusionDetector().getIntrusionStore().getIntrusionRecordForUser(u);
